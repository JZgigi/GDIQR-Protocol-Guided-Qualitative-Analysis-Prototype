    ...
    <section className="project-bar" aria-label="Current project">
      <div className="project-bar-summary">
        <div>
          ...
          <h2 className="project-bar-title">{projectTitle || "Untitled GDI-QR project"}</h2>
          <p className="small">
            {dataSuitabilityConfirmed ? "Data suitability confirmed" : "Data suitability needs confirmation"}
            {projectSetupSavedAt
              ? ` · saved ${new Date(projectSetupSavedAt).toLocaleTimeString()}`
              : ""}
          </p>
        </div>
        <div className="project-bar-actions">
          <label className="sr-only" htmlFor="project-switcher">Open project</label>
          <select
      ...
      <div className="project-bar-disclosures">
        <details className="workbook-details project-settings-details">
          ...
          <div className="grid two">
            ...
            <div>
              <label className="label" htmlFor="project-dataset-type">Dataset sensitivity</label>
              <select
                className="select"
                id="project-dataset-type"
                onChange={(event) => onSetDatasetType(event.target.value as DatasetType)}
                value={datasetType}
              >
          ...
          <label className="label" htmlFor="project-researcher-notes">Dataset/source note</label>
          <textarea
            className="textarea"
            id="project-researcher-notes"
            onChange={(event) => onSetProjectResearcherNotes(event.target.value)}
          ...
          <div className={suitabilityBlocked ? "mini-card warning-card" : "mini-card soft"}>
            <span className="label">Data suitability</span>
            <p className="small">
              This release is intended for open, public, anonymised, or otherwise approved data.
            </p>
            {suitabilityBlocked && (
              <p className="small strong-warning">
                Identifiable sensitive data is blocked in cloud-assisted mode. Use an approved secure/local deployment.
              </p>
            )}
            <label className="scope-option">
              <input
                ...
                onChange={(event) => onSetDataSuitabilityConfirmed(event.target.checked)}
                type="checkbox"
              />
          ...
          <div className="button-row">
            <button className="button primary" disabled={isSavingProject} onClick={onSaveProject} type="button">
        ...
        <details
          ...
          onToggle={(event) => onSetCreateProjectExpanded(event.currentTarget.open)}
        >
          <summary>Create new project</summary>
          <div className="grid two">
            <div>
              <label className="label" htmlFor="new-project-title">Project title</label>
              <input className="field" id="new-project-title" onChange={(event) => onSetNewProjectTitle(event.target.value)} value={newProjectTitle} />
            </div>
            <div>
              <label className="label" htmlFor="new-dataset-type">Dataset sensitivity</label>
              <select className="select" id="new-dataset-type" onChange={(event) => onSetNewDatasetType(event.target.value as DatasetType)} value={newDatasetType}>
                <option value="open">Open / public dataset</option>
                <option value="anonymised">Anonymised or de-identified dataset</option>
                <option value="identifiable_sensitive">Identifiable sensitive data</option>
              </select>
            ...
            <div>
              <label className="label" htmlFor="new-research-question">Research question</label>
              <textarea className="textarea" id="new-research-question" onChange={(event) => onSetNewResearchQuestion(event.target.value)} value={newResearchQuestion} />
            </div>
            <div>
              <label className="label" htmlFor="new-study-description">Domains of investigation</label>
              <textarea className="textarea" id="new-study-description" onChange={(event) => onSetNewStudyDescription(event.target.value)} value={newStudyDescription} />
            </div>
          </div>
          <label className="label" htmlFor="new-researcher-notes">Dataset/source note</label>
          <textarea
            className="textarea"
          ...
          <div className="mini-card soft">
            <label className="scope-option">
              <input
                ...
                disabled={newDatasetType === "identifiable_sensitive" && !isLocalOnlyMode}
                onChange={(event) => onSetNewDataSuitabilityConfirmed(event.target.checked)}
                type="checkbox"
              />
              I confirm the dataset is suitable for this release.
            </label>
          </div>
          <button className="button primary" disabled={isCreatingProject} onClick={onCreateProject} type="button">
